...
const CarceMailView: React.FC<CarceMailViewProps> = ({ config, setConfig, history, setHistory, googleConfig }) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Corrected: using .text property from GenerateContentResponse
      const extractionResponse = await ai.models.generateContent({ 
        model: 'gemini-3-flash-preview', 
        contents: `Extract search query from: "${prompt}". Just the query.` 
      });
      const extractionText = extractionResponse.text || prompt;

      // Corrected: using searchGmail method from googleApi
      const messages = await googleApi.searchGmail(config.accessToken!, extractionText.trim(), 5);
      ...
      // Corrected: using getGmailMessage method from googleApi
      ...
      const answerResponse = await ai.models.generateContent({ 
        model: 'gemini-3-flash-preview', 
        contents: `Answer based on these emails:\n${snippets}\nUser asks: "${prompt}". BE FRIENDLY, USE PARAGRAPHS, REMOVE ASTERISKS.` 
      });
      const answerText = answerResponse.text || 'Sin respuesta.';
      ...
      // Corrected: using spreadsheetId from GoogleConfig and appendRow from googleApi
